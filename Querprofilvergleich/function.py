...
def profiles_generator(path_banks, list_of_profiles, markersize, marker, outputfolder):
    ...
    for sec in stamm_profiles:
        ...
        for _file in list_of_profiles:
            ...
            try:  # tries to take section (if it exist in the dataset)
                ...
                # plt.scatter(references, reference_banks.bedelevation, marker="^", edgecolors='c', label='Stamm
                # Punkte')
                i += 1
        ...
        plt.title(title)
        out = outputfolder + '/' + 'km_' + str(integer) + '_' + str(decimal)
